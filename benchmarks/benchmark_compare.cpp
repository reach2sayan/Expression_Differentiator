...
static void BM_Ours_Reverse_F1(benchmark::State &state) {
    double xv = 1.25; benchmark::DoNotOptimize(xv);
    auto x = PV(xv, 'x');
    auto expr = exp(x) * sin(x) + x * x * x + 2.0 * x;
...
static void BM_Ours_Reverse_F2(benchmark::State &state) {
    ...
    benchmark::DoNotOptimize(xv); benchmark::DoNotOptimize(yv);
    auto x = PV(xv, 'x');
    auto y = PV(yv, 'y');
    auto expr = x * y + sin(x) + y * y + exp(x + y);
    run_our_reverse(state, expr);
...
static void BM_Ours_Reverse_F4(benchmark::State &state) {
    ...
    benchmark::DoNotOptimize(xv); benchmark::DoNotOptimize(yv);
    benchmark::DoNotOptimize(zv); benchmark::DoNotOptimize(wv);
    auto x = PV(xv, 'x'); auto y = PV(yv, 'y');
    auto z = PV(zv, 'z'); auto w = PV(wv, 'w');
    auto expr = (x + y) * (z - w) + exp(x * z) + sin(y * w) + x * y * z * w;
    run_our_reverse(state, expr);
}
...
static void BM_Ours_Reverse_T1(benchmark::State &state) {
    double xv = 2.0; benchmark::DoNotOptimize(xv);
    auto x = PV(xv, 'x');
...
static void BM_Ours_Forward_TMulti3(benchmark::State &state) {
    ...
    auto expr = 1.0 + x + y + z + x*y + y*z + x*z + x*y*z + exp(x/y + y/z);
...
static void BM_Ours_Reverse_TMulti3(benchmark::State &state) {
    ...
    benchmark::DoNotOptimize(xv); benchmark::DoNotOptimize(yv); benchmark::DoNotOptimize(zv);
    auto x = PV(xv, 'x'); auto y = PV(yv, 'y'); auto z = PV(zv, 'z');
    auto expr = PC(1.0) + x + y + z + x*y + y*z + x*z + x*y*z + exp(x/y + y/z);
    run_our_reverse(state, expr);
}
...
static void BM_AD_Reverse_TMulti3(benchmark::State &state) {
    ...
    for (auto _ : state) {
        var u = 1.0 + x + y + z + x*y + y*z + x*z + x*y*z + exp(x/y + y/z);
...
static void BM_Ours_Taylor_T4th(benchmark::State &state) {
    double x0 = T4_X0; benchmark::DoNotOptimize(x0);
    auto x = PV(x0, 'x');
    auto expr = sin(x);
    for (auto _ : state) {
...
static void BM_AD_Forward_THess(benchmark::State &state) {
    ...
    using autodiff::detail::wrt;
    using autodiff::detail::at;
    auto f = [](dual2nd x, dual2nd y) -> dual2nd { return x * x + x * y + y * y; };
    dual2nd x = 2.0, y = 3.0;
    for (auto _ : state) {
        benchmark::DoNotOptimize(x);
        benchmark::DoNotOptimize(y);
